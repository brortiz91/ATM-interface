...
class ATM(Frame):
	...
	def __init__(self, bank):
		...
		#Create and add the windgets to the frame.
		...
		self._loginButton = Button(self, text = "Log In", command = self._login)
	...
	def _login(self):
		...
		if self._account:
			if name == self._account.getName():
				...
				self._loginButton["text"] = "Log Out"
				...
				self._statusVar.set("ERROR. NAME NOT RECOGNIZED.")
		else:
			self._statusVar.set("ERROR. PIN NOT RECOGNIZED.")
...
#Top level functions
def main():	
	"""Instatiates a Bank and an ATM to run it"""
	bank = Bank("bank.dat")
